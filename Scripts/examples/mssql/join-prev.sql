with ordered_gage_record
as
(
select 
ROW_NUMBER (order pcn,gage_no,add_date) row_num,
pcn,
gage_no,
add_date,
next_calibration_date
from quality_v_gage_record n  
)
select p.pcn,p.gage_no,p.next_calibrition_date,n.add_date
from ordered_gage_record p
inner join ordered_gage_record n
on p.pcn = n.pcn 
and p.gage_no = n.gage_no 
and p.row_num=n.row_num+1
...
@prev_period prev_period,@start_period start_period,@period period,
@first_period first_period,@end_period end_period,@max_fiscal_period max_fiscal_period;
...
--	select * from Plex.accounting_account_year_category_type y where pcn = 123681 and year =2022 order by account_no desc
