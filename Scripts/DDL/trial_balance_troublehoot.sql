...
/*
select count(*) from Archive.account_period_balance_05_12_2022 -- 123,659
select count(*) from Archive.account_period_balance_05_12_2022_bad -- 766589
select *

--into Archive.account_period_balance_05_12_2022 -- 160,655 
into Archive.account_period_balance_05_12_2022_bad -- 766589
...
--select * from Plex.accounting_balance_update_period_range r 
-- update Plex.accounting_balance_update_period_range set period_start = 202105, period_end = 202204 where id in (6,7)
...
-- delete from Plex.account_period_balance where period > 202203
--select *
...
where account_no like '73250%' --22 73250 
select * from Plex.account_period_balance apb -- 131,900, 123,615
where account_no like '73250-%' and period = 202104 and pcn = 123681 order by account_no  --22 73250 
select * from Plex.accountING_balance apb -- 131,900, 123,615
where account_no  like '73250%' AND pcn = 123681 order by account_no  --22 73250 
...
--are there any new accounts to add?
	    ...
	    0 balance,
	    0 ytd_balance
	    -- select * from Plex.accounting_account where pcn = 123681 and account_no like ''
	...
	select * from Plex.account_period_balance apb -- 131,900, 123,615
where account_no like '73250%' --22 73250 
and pcn = 123681 and period = 202104 order by account_no 
...
--		with account_period(pcn,account_no,period)
	...
	--	select * from period_balance
		...
		--	and p.period=b.period -- WHY IS THIS REMOVED?  because it's in the WHERE clause now and should be @prev_period not b.period.
			...
			and b.account_no like '73250-%' --and period = @period and pcn = @pcn   order by account_no  --22 73250 
		
			
			where p.period = @prev_period  
			and b.account_no like '73250-%' and period = @period and pcn = @pcn   order by account_no  --22 73250 
	--	where account_no like '73250-%' and period = @period and pcn = @pcn   order by account_no  --22 73250 
...
*/
   ...
   -- where pcn=123681 and period=202101 and debit=0 and ytd_debit = 0 and credit = 0 and ytd_credit =0 and balance = 0 and ytd_balance =0  -- 3,815
    insert into Plex.account_period_balance 
		...
		select a.*
		from Plex.accounting_account a   
		left outer Plex.accounting_account_year_category_type y
		on a.pcn = y.pcn 
		and a.account_no =y.account_no
		where a.[year] = (@prev_period/100)  
